...
export default function TempatUsahaPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = { ...form, berjualan_sejak: berjualanDate ? format(berjualanDate, 'yyyy-MM-dd') : form.berjualan_sejak };
                ...
                <CardContent className="p-3">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Rp {kd.harga_normal.toLocaleString('id-ID')} - Rp {kd.harga_mahal.toLocaleString('id-ID')}</p>
                      <span className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-full font-medium ${kd.is_active ? 'bg-green-500/15 text-green-600' : 'bg-muted text-muted-foreground'}`}>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          <SelectTrigger className="w-full sm:w-36 h-9"><SelectValue placeholder="Semua Status" /></SelectTrigger>
          <SelectContent>
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <span className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-full font-medium ${t.is_active ? 'bg-green-500/15 text-green-600' : 'bg-muted text-muted-foreground'}`}>
